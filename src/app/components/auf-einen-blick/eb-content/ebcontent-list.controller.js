 ...
 */
class EbContentListController {

	constructor($state, $stateParams, $mdDialog, EbContent, EbContentTranslation) {
		this.$state = $state;
	...
	init() {
		...
		this.EbContent.find(q, function(res) {
			this.entities = res;
		}.bind(this));
	}
	...
	deleteEntity(entity) {
		this.deleteEntityTranslation(entity.translations[0])
			.then(function(res) {
				return this.EbContent.deleteById({id: entity.id}).$promise;
			}.bind(this))
	...
	deleteEntityTranslation(translation) {
		...
		return this.EbContentTranslation.deleteById({id: translation.id}).$promise;
	}
...
EbContentListController.$inject = ['$state', '$stateParams', '$mdDialog', 'EbContent', 'EbContentTranslation'];
export default EbContentListController;
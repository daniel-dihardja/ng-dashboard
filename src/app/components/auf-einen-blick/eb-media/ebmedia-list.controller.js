 ...
 */
class EbMediaListController {

	constructor($state, $stateParams, $mdDialog, EbMedia, EbMediaTranslation) {
		this.$state = $state;
	...
	init() {
		...
		this.EbMedia.find(q, function(res) {
			this.entities = res;
		}.bind(this));
	}
	...
	deleteEntity(entity) {
		this.deleteEntityTranslation(entity.translations[0])
			.then(function(res) {
				return this.EbMedia.deleteById({id: entity.id}).$promise;
			}.bind(this))
	...
	deleteEntityTranslation(translation) {
		...
		return this.EbMediaTranslation.deleteById({id: translation.id}).$promise;
	}
...
EbMediaListController.$inject = ['$state', '$stateParams', '$mdDialog', 'EbMedia', 'EbMediaTranslation'];
export default EbMediaListController;